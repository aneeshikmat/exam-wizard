...
if(totalOfQuestion==markedQuestionNumber||!settings.nextOption.allowadNext){$(settings.nextOption.nextSelector).addClass('disabled')}else{$(settings.nextOption.nextSelector).removeClass('disabled')}});$(document).on('click',settings.finishOption.finishBtnSelector,function(){if(settings.finishOption.enableModal){$(settings.finishOption.finishModalAnswerd).html(getTotalOfAnswerdQuestion());$(settings.finishOption.finishModalMarked).html(getTotalOfMarkedQuestion());$(settings.finishOption.finishModalRemaining).html(getTotalOfRemainingQuestion());$(settings.finishOption.finishModalTarget).modal('show')}
settings.finishOption.callBack()});$(document).on('click',settings.quickAccessOption.quickAccessPrevSelector,function(){if(settings.quickAccessOption.enableAccessSection){quickAccessAction('Prev');setQuickAccessResponseForAllData();settings.quickAccessOption.prevCallBack()}});$(document).on('click',settings.quickAccessOption.quickAccessNextSelector,function(){if(settings.quickAccessOption.enableAccessSection){quickAccessAction('Next');setQuickAccessResponseForAllData();settings.quickAccessOption.nextCallBack()}});$(document).on('change select keyup','['+settings.alternateNameAttr+']',function(){var currentQNumber=_getCurrentQuestionNumber();var answerdValue=getAllAnswerdValue(!0,!0,$(this).attr('name'));if(!$.isEmptyObject(answerdValue)){_convertAnswerdToReadAnswerd(answerdValue);if(settings.cookiesOption.enableCookie){handlingCookies(answerdValue)}}else{setQuickAccessResponseValue(null,currentQNumber);if(settings.cookiesOption.enableCookie){deleteCookie($(this).attr('name'))}}});function getAllAnswerdValue(groupingData=!1,getAlternateAnswerdOnly=!0,fieldName=!1){var formsValue=_getAllFormData(getAlternateAnswerdOnly);if(fieldName){formsValue=$('[name="'+fieldName+'"]').serializeArray()}
...
if(settings.cookiesOption.enableCookie){loadAnswersFromCookies()}
...
return{getMarkedQuestion:function(){return _getMarkedValue()},getAnswerdValue:function(groupingData=!0,fieldWithAlternateNameOnly=!0){return getAllAnswerdValue(groupingData,fieldWithAlternateNameOnly)},getTotalOfAnswerdValue:function(){return getTotalOfAnswerdQuestion()},getTotalOfMarkedValue:function(){return getTotalOfMarkedQuestion()},getTotalOfRemainingValue:function(){return getTotalOfRemainingQuestion()},getCurrentQuestionNumber:function(){return _getCurrentQuestionNumber()},getMarkedValues:function(){return _getMarkedValue()},getAllFormData:function(){return _getAllFormData()},}}}(jQuery))
